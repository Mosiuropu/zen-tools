...
const TECHNIQUES = {
  box: {
    ...
    steps: [
      ...
      { label: 'Hold', duration: 4, scale: 'scale-100', color: 'bg-[var(--color-claude-bg-light)] dark:bg-[var(--color-claude-bg-dark)]0' }
    ]
  },
  relax: {
    name: '4-7-8 Relax',
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-12 animate-in fade-in duration-700">
        ...
        {Object.entries(TECHNIQUES).map(([key, data]) => (
          <button
            ...
            className={`px-8 py-3 rounded-md font-semibold text-sm transition-all border-2 ${
              technique === key 
                ? 'bg-[var(--color-claude-card-light)] dark:bg-[var(--color-claude-card-dark)]/10 border-orange-500 text-[var(--color-claude-accent-primary)]' 
      ...
      <div className="bg-[var(--color-claude-card-light)] dark:bg-[var(--color-claude-bg-dark)] dark:bg-[var(--color-claude-card-dark)] border border-[var(--color-claude-border-light)] dark:border-[var(--color-claude-border-dark)] dark:border-[var(--color-claude-border-light)] dark:border-[var(--color-claude-border-dark)] rounded-[4rem] p-12 md:p-24 shadow-2xl flex flex-col items-center justify-center min-h-[500px] relative overflow-hidden">
        ...
        <div 
          className={`absolute w-64 h-64 md:w-80 md:h-80 rounded-md blur-[80px] opacity-20 transition-all duration-[4000ms] ease-in-out ${isActive ? currentStep.color : 'bg-[var(--color-claude-bg-light)] dark:bg-[var(--color-claude-bg-dark)]0'}`}
        />

        {/* The Breathing Circle */}
        <div className="relative z-10 flex flex-col items-center space-y-12">
          <div 
            className={`w-32 h-32 md:w-48 md:h-48 rounded-md border-8 border-white dark:border-[var(--color-claude-border-light)] dark:border-[var(--color-claude-border-dark)] flex items-center justify-center transition-all duration-[4000ms] ease-in-out shadow-2xl ${isActive ? `${currentStep.scale} ${currentStep.color} border-transparent` : 'bg-[var(--color-claude-border-light)] dark:bg-[var(--color-claude-bg-dark)] dark:bg-[var(--color-claude-bg-dark)] scale-100'}`}
          >
            <div className="text-sm md:text-base font-semibold text-[var(--color-claude-text-light)] dark:text-[var(--color-claude-text-dark)] dark:text-[var(--color-claude-text-dark)] tabular-nums">
              {isActive ? timeLeft : <Wind className="w-12 h-12 md:w-16 md:h-16 text-[var(--color-claude-muted-light)] dark:text-[var(--color-claude-text-dark)] dark:text-[var(--color-claude-text-dark)]" />}
            </div>
          </div>

          <div className="text-center space-y-2">
            <h4 className={`text-sm md:text-base font-semibold tracking-tight transition-all duration-500 ${isActive ? 'text-[var(--color-claude-text-light)] dark:text-[var(--color-claude-text-dark)] dark:text-[var(--color-claude-text-dark)]' : 'text-[var(--color-claude-muted-light)] dark:text-[var(--color-claude-text-dark)] dark:text-[var(--color-claude-text-dark)]'}`}>
              {isActive ? currentStep.label : 'Ready?'}
            </h4>
            <p className="text-[var(--color-claude-muted-light)] dark:text-[var(--color-claude-text-dark)] dark:text-[var(--color-claude-text-dark)]0 font-semibold uppercase tracking-[0.2em] text-xs">
          ...
          <button
            ...
            className={`px-12 py-5 rounded-[2rem] font-semibold text-sm flex items-center gap-3 transition-all active:scale-95 shadow-2xl ${
              isActive 
                ? 'bg-[var(--color-claude-border-light)] dark:bg-[var(--color-claude-bg-dark)] dark:bg-[var(--color-claude-border-dark)] text-[var(--color-claude-text-light)] dark:text-[var(--color-claude-text-dark)] dark:text-[var(--color-claude-text-dark)] hover:bg-red-500/10 hover:text-red-500' 
                : 'zen-btn-primary'
            }`}
          >
