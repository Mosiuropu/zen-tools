...
const TECHNIQUES = {
  box: {
    ...
    steps: [
      ...
      { label: 'Hold', duration: 4, scale: 'scale-100', color: 'bg-[#F9F8F6]0' }
    ]
  },
  relax: {
    name: '4-7-8 Relax',
...
export default function ZenBreathing() {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-12 animate-in fade-in duration-700">
        ...
        {Object.entries(TECHNIQUES).map(([key, data]) => (
          <button
            ...
            className={`px-8 py-3 rounded-2xl font-bold text-sm transition-all border-2 ${
              technique === key 
                ? 'bg-[#D97757]/10 border-blue-500 text-blue-500' 
      ...
      <div className="bg-white dark:bg-[#2A2A29] border border-[#E6E4E0] dark:border-[#3A3A39] rounded-[4rem] p-12 md:p-24 shadow-2xl flex flex-col items-center justify-center min-h-[500px] relative overflow-hidden">
        {/* Background Visualizer */}
        <div 
          className={`absolute w-64 h-64 md:w-80 md:h-80 rounded-full blur-[80px] opacity-20 transition-all duration-[4000ms] ease-in-out ${isActive ? currentStep.color : 'bg-[#F9F8F6]0'}`}
        />

        {/* The Breathing Circle */}
        <div className="relative z-10 flex flex-col items-center space-y-12">
          <div 
            className={`w-32 h-32 md:w-48 md:h-48 rounded-full border-8 border-white dark:border-[#3A3A39] flex items-center justify-center transition-all duration-[4000ms] ease-in-out shadow-2xl ${isActive ? `${currentStep.scale} ${currentStep.color} border-transparent` : 'bg-[#F0EFEA] dark:bg-[#1E1D1B] scale-100'}`}
          >
          ...
          <div className="text-center space-y-2">
            <h4 className={`text-4xl md:text-6xl font-black tracking-tight transition-all duration-500 ${isActive ? 'text-[#2D2D2D] dark:text-[#E8E6E3]' : 'text-slate-300 dark:text-slate-800'}`}>
              {isActive ? currentStep.label : 'Ready?'}
            </h4>
            <p className="text-slate-400 dark:text-[#73716D] font-bold uppercase tracking-[0.2em] text-xs">
          ...
          <button
            ...
            className={`px-12 py-5 rounded-[2rem] font-black text-lg flex items-center gap-3 transition-all active:scale-95 shadow-2xl ${
              isActive 
                ? 'bg-[#F0EFEA] dark:bg-[#3A3A39] text-[#2D2D2D] dark:text-slate-100 hover:bg-red-500/10 hover:text-red-500' 
                : 'bg-[#D97757] text-white hover:bg-blue-500'
            }`}
          >
