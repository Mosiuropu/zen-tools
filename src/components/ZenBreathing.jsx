...
const TECHNIQUES = {
  box: {
    ...
    steps: [
      ...
      { label: 'Hold', duration: 4, scale: 'scale-100', color: 'bg-stone-500' }
    ]
  },
  relax: {
    name: '4-7-8 Relax',
...
export default function ZenBreathing() {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-12 animate-in fade-in duration-700">
        ...
        {Object.entries(TECHNIQUES).map(([key, data]) => (
          <button
            ...
            className={`px-8 py-3 rounded-md font-semibold text-sm transition-all border-2 ${
              technique === key 
                ? 'bg-stone-900/10 border-orange-500 text-orange-500' 
      ...
      <div className="bg-white dark:bg-stone-900 border border-stone-200 dark:border-stone-800 rounded-[4rem] p-12 md:p-24 shadow-2xl flex flex-col items-center justify-center min-h-[500px] relative overflow-hidden">
        {/* Background Visualizer */}
        <div 
          className={`absolute w-64 h-64 md:w-80 md:h-80 rounded-md blur-[80px] opacity-20 transition-all duration-[4000ms] ease-in-out ${isActive ? currentStep.color : 'bg-stone-500'}`}
        />

        {/* The Breathing Circle */}
        <div className="relative z-10 flex flex-col items-center space-y-12">
          <div 
            className={`w-32 h-32 md:w-48 md:h-48 rounded-md border-8 border-white dark:border-stone-800 flex items-center justify-center transition-all duration-[4000ms] ease-in-out shadow-2xl ${isActive ? `${currentStep.scale} ${currentStep.color} border-transparent` : 'bg-stone-100 dark:bg-stone-950 scale-100'}`}
          >
            <div className="text-sm md:text-base font-semibold text-stone-900 dark:text-stone-100 tabular-nums">
              {isActive ? timeLeft : <Wind className="w-12 h-12 md:w-16 md:h-16 text-stone-300 dark:text-stone-800" />}
            </div>
          </div>

          <div className="text-center space-y-2">
            <h4 className={`text-sm md:text-base font-semibold tracking-tight transition-all duration-500 ${isActive ? 'text-stone-900 dark:text-stone-50' : 'text-stone-300 dark:text-stone-800'}`}>
              {isActive ? currentStep.label : 'Ready?'}
            </h4>
            <p className="text-stone-400 dark:text-stone-500 font-semibold uppercase tracking-[0.2em] text-xs">
          ...
          <button
            ...
            className={`px-12 py-5 rounded-[2rem] font-semibold text-sm flex items-center gap-3 transition-all active:scale-95 shadow-2xl ${
              isActive 
                ? 'bg-stone-100 dark:bg-stone-800 text-stone-900 dark:text-stone-100 hover:bg-red-500/10 hover:text-red-500' 
                : 'zen-btn-primary'
            }`}
          >
      ...
      <div className="bg-stone-900/5 dark:bg-blue-950/20 border border-orange-500/20 rounded-md p-6 flex items-start gap-4">
        <Info className="w-6 h-6 text-orange-500 mt-1 shrink-0" />
