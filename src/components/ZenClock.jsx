  ...
  return (
    <div className="max-w-4xl mx-auto space-y-12 animate-in fade-in duration-700">
      <div className="text-center space-y-4">
        <h3 className="text-sm font-semibold uppercase tracking-[0.3em] text-[var(--color-zen-accent-primary)]">Temporal Zen</h3>
      ...
      <div className="flex flex-col lg:flex-row items-center justify-center gap-16 md:gap-24">
        ...
        <div className="bg-[var(--color-zen-card-light)] dark:bg-[var(--color-zen-bg-dark)] dark:bg-[var(--color-zen-card-dark)] border border-[var(--color-zen-border-light)] dark:border-[var(--color-zen-border-dark)] dark:border-[var(--color-zen-border-light)] dark:border-[var(--color-zen-border-dark)] rounded-[3rem] p-10 md:p-16 shadow-2xl flex flex-col items-center justify-center space-y-4 min-w-[320px]">
          <div className="text-[10px] font-semibold uppercase tracking-[0.4em] text-[var(--color-zen-muted-light)] dark:text-[var(--color-zen-muted-dark)]">Current Time</div>
          <div className="text-base md:text-5xl font-semibold text-[var(--color-zen-text-light)] dark:text-[var(--color-zen-text-dark)] dark:text-[var(--color-zen-text-dark)] tabular-nums tracking-tighter">
            {format(time, 'HH:mm')}
            <span className="text-[var(--color-zen-accent-primary)] ml-1 opacity-80">{format(time, ':ss')}</span>
          </div>
          <div className="text-sm font-semibold text-[var(--color-zen-text-light)] dark:text-[var(--color-zen-text-dark)]0 uppercase tracking-widest">
        ...
        <div className="flex flex-col items-center">
          {/* Clock Face */}
          <div className="relative w-64 h-64 md:w-80 md:h-80 bg-[var(--color-zen-card-light)] dark:bg-[var(--color-zen-bg-dark)] dark:bg-[var(--color-zen-bg-dark)] rounded-md border-8 border-stone-100 dark:border-stone-900 shadow-2xl flex items-center justify-center z-10">
            {/* Center Point */}
            <div className="absolute w-4 h-4 bg-orange-500 rounded-md z-40 shadow-sm" />
            
            {/* Hour Hand */}
            <div 
              className="absolute w-1.5 h-20 md:h-24 bg-[var(--color-zen-border-light)] dark:bg-[var(--color-zen-bg-dark)] dark:bg-[var(--color-zen-border-dark)] rounded-md origin-bottom transition-all duration-300 ease-out"
            ...
            {[...Array(12)].map((_, i) => (
              <div 
                key={i}
                className="absolute w-1 h-3 bg-[var(--color-zen-border-light)] dark:bg-[var(--color-zen-bg-dark)] dark:bg-[var(--color-zen-border-dark)]"
          ...
          <div className="relative flex flex-col items-center mt-[-4px]">
            ...
            <div className="animate-zen-pendulum origin-top flex flex-col items-center">
               ...
               <div className="w-10 h-10 md:w-14 md:h-10 bg-stone-400 dark:bg-[var(--color-zen-bg-dark)] rounded-md border-4 border-stone-100 dark:border-stone-900 shadow-sm flex items-center justify-center">
