...
export default function ZenNotes() {
  ...
  return (
    <div className={`max-w-4xl mx-auto space-y-8 transition-all duration-700 ${isFocusMode ? 'mt-[-80px]' : ''}`}>
      <div className={`flex justify-between items-center transition-opacity duration-500 ${isFocusMode ? 'opacity-0 h-0 overflow-hidden' : 'opacity-100'}`}>
        <div className="space-y-1">
          <h2 className="text-base font-semibold text-[var(--color-zen-text-light)] dark:text-[var(--color-zen-text-dark)] dark:text-[var(--color-zen-text-dark)] flex items-center gap-3">
            <FileText className="w-8 h-8 text-[var(--color-zen-text-light)] dark:text-[var(--color-zen-text-dark)] dark:text-[var(--color-zen-accent-primary)]" />
            Zen Notes
          </h2>
          <p className="text-[var(--color-zen-text-light)] dark:text-[var(--color-zen-text-dark)]0 dark:text-[var(--color-zen-text-dark)] font-medium">A sanctuary for your thoughts. Markdown supported.</p>
        </div>
        
        <div className="flex gap-4">
          <button
            onClick={downloadNotes}
            className="p-4 rounded-md bg-[var(--color-zen-card-light)] dark:bg-[var(--color-zen-bg-dark)] dark:bg-[var(--color-zen-card-dark)] border border-[var(--color-zen-border-light)] dark:border-[var(--color-zen-border-dark)] dark:border-[var(--color-zen-border-light)] dark:border-[var(--color-zen-border-dark)] text-[var(--color-zen-muted-light)] dark:text-[var(--color-zen-text-dark)] dark:text-[var(--color-zen-text-dark)]0 hover:text-[var(--color-zen-text-light)] dark:text-[var(--color-zen-text-dark)] dark:hover:text-[var(--color-zen-text-light)] dark:text-[var(--color-zen-text-dark)] dark:hover:text-[var(--color-zen-text-light)] dark:text-[var(--color-zen-text-dark)] hover:border-orange-200 dark:hover:border-[var(--color-zen-border-light)] dark:border-[var(--color-zen-border-dark)] transition-all shadow-sm"
      ...
      <div className={`relative group bg-[var(--color-zen-card-light)] dark:bg-[var(--color-zen-bg-dark)] dark:bg-[var(--color-zen-card-dark)] border border-[var(--color-zen-border-light)] dark:border-[var(--color-zen-border-dark)] dark:border-[var(--color-zen-border-light)] dark:border-[var(--color-zen-border-dark)] rounded-[2.5rem] p-8 md:p-12 shadow-2xl transition-all duration-700 ${isFocusMode ? 'bg-opacity-0 dark:bg-opacity-0 border-none shadow-none p-0 max-w-none' : ''}`}>
        <button
          onClick={() => setIsFocusMode(!isFocusMode)}
          className={`absolute right-8 top-8 p-3 rounded-md bg-[var(--color-zen-bg-light)] dark:bg-[var(--color-zen-bg-dark)] dark:bg-[var(--color-zen-bg-dark)] text-[var(--color-zen-muted-light)] dark:text-[var(--color-zen-text-dark)] dark:text-[var(--color-zen-muted-dark)] hover:text-[var(--color-zen-text-light)] dark:text-[var(--color-zen-text-dark)] dark:hover:text-[var(--color-zen-accent-primary)] transition-all z-10 ${isFocusMode ? 'fixed right-10 top-10 opacity-20 hover:opacity-100' : 'opacity-0 group-hover:opacity-100'}`}
        >
        ...
        <textarea
          ...
          className={`w-full min-h-[60vh] bg-transparent border-none text-[var(--color-zen-text-light)] dark:text-[var(--color-zen-text-dark)] dark:text-stone-200 text-base leading-relaxed focus:ring-0 placeholder-stone-200 dark:placeholder-stone-800 resize-none font-serif selection:bg-orange-500/30 ${isFocusMode ? 'text-sm py-20 px-4 md:px-20 text-center max-w-5xl mx-auto block' : ''}`}
          spellCheck="false"
        />

        {!isFocusMode && (
          <div className="mt-8 pt-8 border-t border-stone-100 dark:border-[var(--color-zen-border-light)] dark:border-[var(--color-zen-border-dark)]/50 flex justify-between items-center text-[10px] font-semibold uppercase tracking-widest text-[var(--color-zen-muted-light)] dark:text-[var(--color-zen-text-dark)] dark:text-[var(--color-zen-muted-dark)]">
